...
namespace DealerCore.Util
{
    public class AuthMiddleware
    {
        ...
        public AuthMiddleware(RequestDelegate next, IConfiguration configuration, IMemoryCache memoryCache)
        {
            _next = next;
            _cache = memoryCache;
        ...
        public async Task ProcessAsync(HttpContext context)
        {
            try
            {
                ...
                if (context.Request.Query.Keys.Count == 0)
                {
                    ...
                    {
                        _cache.Remove("count");
                        context.Response.Redirect(Configuration["openIdInfo:redirectUri"]);
                    }
                }
        ...
        private async Task ValidateAsyncToken(HttpContext context, string accessToken, string expiresIn)
        {
            ...
            var certCollection = store.Certificates.Find(X509FindType.FindByThumbprint, Configuration["openIdInfo:CertificateValue"], false);
